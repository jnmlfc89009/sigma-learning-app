...
import { MessageSquare, Send, X, Bot, User, Headphones, Sparkles, HelpCircle, ArrowRight, ShieldCheck, CheckCircle, Ticket, Settings, Phone, ArrowLeft } from 'lucide-react';
...
export default function ChatbotWidget({ user }: ChatbotWidgetProps) {
  ...
  // WhatsApp Support Configuration & Custom State Integration
  const [waNumber, setWaNumber] = useState(() => safeStorage.getItem('sigma_wa_number') || '15550199888');
  const [waText, setWaText] = useState(() => safeStorage.getItem('sigma_wa_text') || 'Hello Sigma Support! I am studying on the platform and would love some help or guidance.');
  const [showWaConfig, setShowWaConfig] = useState(false);
  const [tempWaNumber, setTempWaNumber] = useState(waNumber);
  const [tempWaText, setTempWaText] = useState(waText);
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: "wel_1",
      sender: 'bot',
      text: `Hello ${user.username}! I am the Sigma Learning Assistant. I can help resolve curriculum queries, explain double-entry mechanics, or support upgrades. Please select an option below${user.tier === 'magnate' ? ', ask about our WhatsApp Support 🟢,' : ''} or type a custom question!`,
    ...
    <>
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="bg-slate-900 text-white px-5 py-4 flex items-center justify-between border-b border-slate-950">
            ...
            <div className="flex items-center gap-2">
              {user.tier === 'magnate' && (
                <button
                  onClick={() => {
                    setShowWaConfig(prev => !prev);
                    setHasSavedConfig(false);
                  }}
                  className={`p-1 text-xs shrink-0 rounded-lg transition ${showWaConfig ? 'text-emerald-400 bg-slate-800' : 'text-slate-400 hover:text-white'}`}
                  title="Configure WhatsApp settings"
                >
                  <Settings className="w-4 h-4 whitespace-nowrap" />
                </button>
              )}
              <button
                onClick={() => {
          ...
          {showWaConfig ? (
            /* WhatsApp Config form overlay */
            <div className="flex-grow flex flex-col justify-between bg-white text-slate-800 p-5 overflow-y-auto">
              <div className="space-y-4">
                <div className="flex items-center gap-2 border-b pb-3">
                  <Phone className="w-4 h-4 text-emerald-600 shrink-0" />
                  <div>
                    <h4 className="font-display font-black text-xs text-slate-900 uppercase">WhatsApp Hotline Settings</h4>
                    <p className="text-[9px] text-slate-500 font-sans">Deploy client-side live support configurations</p>
                  </div>
                </div>

                <div className="space-y-1 bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block font-mono">WhatsApp Hotline Number</label>
                  <input
                    type="text"
                    required
                    placeholder="e.g. 15550199888"
                    value={tempWaNumber}
                    onChange={(e) => setTempWaNumber(e.target.value.replace(/\D/g, ''))}
                    className="w-full text-xs px-3 py-2 border border-slate-200 rounded-xl bg-white focus:outline-none focus:ring-1 focus:ring-emerald-500 text-slate-800 font-mono"
                  />
                  <span className="text-[9px] text-slate-400 font-sans block leading-snug mt-1">
                    Enter numbers only (with international dialing code). Example: <b>15550199888</b>.
                  </span>
                </div>

                <div className="space-y-1 bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block font-mono">Custom Message Template</label>
                  <textarea
                    rows={3}
                    placeholder="E.g. Hello, support! Help me with course questions."
                    value={tempWaText}
                    onChange={(e) => setTempWaText(e.target.value)}
                    className="w-full text-xs p-2.5 border border-slate-200 rounded-xl bg-white focus:outline-none focus:ring-1 focus:ring-emerald-500 text-slate-800 leading-normal"
                  />
                  <span className="text-[9px] text-slate-400 font-sans block leading-snug mt-1">
                    Preloaded chat message delivered to your WhatsApp agent queue.
                  </span>
                </div>

                {hasSavedConfig && (
                  <div className="p-2.5 bg-emerald-50 border border-emerald-100 text-emerald-800 rounded-xl text-[10px] font-sans flex items-center gap-1.5 animate-pop">
                    <CheckCircle className="w-3.5 h-3.5 shrink-0 animate-bounce" />
                    <span>WhatsApp support configuration successfully saved in persistent browser cache.</span>
                  </div>
                )}
              </div>

              <div className="pt-4 border-t border-slate-100 flex gap-2 shrink-0 bg-white mt-4">
                <button
                  type="button"
                  onClick={() => setShowWaConfig(false)}
                  className="flex-1 py-2 text-xs font-bold text-slate-500 hover:bg-slate-150 border border-slate-200 rounded-xl cursor-pointer"
                >
                  Back
                </button>
                <button
                  type="button"
                  onClick={() => {
                    safeStorage.setItem('sigma_wa_number', tempWaNumber);
                    safeStorage.setItem('sigma_wa_text', tempWaText);
                    setWaNumber(tempWaNumber);
                    setWaText(tempWaText);
                  ...
                  className="flex-grow bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs uppercase tracking-wide rounded-xl border-b-4 border-emerald-900 shadow py-2 px-3 cursor-pointer text-center"
                >
                  Save Config
                </button>
              </div>
            ...
            <>
              ...
              <div className="p-3 bg-white border-t border-slate-100 space-y-2 shrink-0">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {FAQ_OPTIONS.map((opt) => (
                    <button
                      ...
                      className="text-[10px] font-sans font-bold bg-slate-100 text-slate-700 hover:bg-indigo-50 hover:text-indigo-700 px-2.5 py-1.5 rounded-lg border border-slate-200 transition text-left cursor-pointer"
                    >
                      {opt.q}
                    </button>
                  ))}
                ...
                {/* WHATSAPP SUPPORT DIRECT WEB LINK - CONNECT ON WHATSAPP (MAGNATES ONLY) */}
                {user.tier === 'magnate' && (
                  <div className="pt-2 border-t border-dashed border-slate-200 mt-2 flex flex-col gap-1.5">
                    <a
                      href={`https://wa.me/${waNumber}?text=${encodeURIComponent(waText)}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-full bg-emerald-600 hover:bg-emerald-750 text-white font-extrabold text-[11px] py-2 px-3 rounded-xl border-b-2 border-emerald-950 shadow transition flex items-center justify-center gap-1.5 uppercase tracking-wide cursor-pointer text-center"
                    >
                      <span className="w-1.5 h-1.5 rounded-full bg-emerald-250 animate-pulse inline-block" />
                  ...
                  <div className="pt-1.5 border-t border-slate-100 mt-1">
                    {hasInteracted ? (
                      <button
                        ...
                        className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-[11px] py-1.5 px-3 rounded-xl border-b-2 border-indigo-900 shadow transition flex items-center justify-center gap-1.5 uppercase tracking-wide cursor-pointer"
                      >
